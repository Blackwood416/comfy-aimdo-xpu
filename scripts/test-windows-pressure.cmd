@echo off
rem Host-only regression tests using the production WDDM pressure code.
setlocal EnableExtensions
for %%I in ("%~dp0..") do set "ROOT_DIR=%%~fI"
set "BUILD_DIR=%ROOT_DIR%\build\windows-pressure-unit"
...
cl.exe /nologo /O2 /MD /W3 /DAIMDO_XPU /I"%ROOT_DIR%\src" ^
    "%ROOT_DIR%\tests\windows_pressure_unit.c" ^
    /Fo"%BUILD_DIR%\\" /Fe"%BUILD_DIR%\windows_pressure_unit.exe" ^
    /link dxgi.lib dxguid.lib
if errorlevel 1 exit /b 1
"%BUILD_DIR%\windows_pressure_unit.exe"
if errorlevel 1 exit /b 1
endlocal
